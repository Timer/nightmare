...
function appReady(parent) {
  ...
  parent.respondTo('javascript', function(src, id, done) {
    var response = (event, response) => {
      renderer.removeListener(`error_${id}`, error);
      renderer.removeListener(`log_${id}`, log);
      done(null, response);
    };

    var error = (event, error) => {
      renderer.removeListener(`log_${id}`, log);
      renderer.removeListener(`response_${id}`, response);
      done(error);
    };

    var log = (event, args) => parent.emit.apply(parent, ['log'].concat(args));

    renderer.once(`response_${id}`, response);
    renderer.once(`error_${id}`, error);
    renderer.on(`log_${id}`, log);
